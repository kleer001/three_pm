...
in-game "day"**; the only constant is a kids' GPS smart-watch whose compass
*yearns* toward the wearer's house. The bearing home is a pure function of the
day's seed, so it **changes every day** — which is the in-fiction proof the day
reset. Roguelite: dying resets the kid to another 3pm (groundhog-day), spending
banked currency on **per-hero permanent upgrades**, with new heroes **unlocked by
run count**. Win = reach the home-band edge in the watch's bearing direction.

## Locked high-level decisions
- Top-down, **real-time action**; WASD move, mouse aim (360°).
- **Single fixed-size suburban map** per day; camera scrolls + clamps to bounds.
- **Open-tile suburb generation**: jittered street grid → blocks/lots → houses →
  cellular-automata **decay pass** → connectivity repair → home band + start.
- **Win = touch the home-band edge** in the bearing direction. **No boss**; pure
  navigation. Death (hp ≤ 0) is the only other end state.
...
- **Full HUD**: HP/mana bars, signature cooldown, **edge-arrow compass**, scrap
  counter, minimap.
...
|---|---|---|---|
| 01 | Core engine + run loop | Engine skeleton; no content | Fixed-timestep `Loop.start`, `Scene{enter/update/render/exit}`, scene FSM, seeded `makeRng` + sub-streams, daily compass bearing, `runState` + `meta/save` tiers, `events` pub/sub |
| 02 | World / level structure | One fixed suburban map per day | `generate(seed,params)->Level`, `Level` shape, tile-type enum + `walkable`, gen pipeline (grid→houses→CA decay→repair), home band, win check (player tile ∈ homeBand), tile/world coord convention |
...
| 09 | Presentation | Render / camera / HUD / audio | `render(alpha)` interpolation + 4-layer order + y-sort, `sprites.json` + `sprite` anim state + `advanceSprite`, camera `follow`/clamp + cull, HUD element→source table + edge-arrow compass math, `audio` API + `audio.json` + per-scene music + event SFX |
...
| Seeded `makeRng` + sub-streams (`gen`/`spawns`/`loot`/`compass`) | 01 |
| `runState` tier + `meta/save` blob shape + `KEY` + `events` | 01 (save API + schema detailed in 08) |
| Daily compass bearing (pure fn of seed) | 01 |
| `generate(seed,params)->Level`, `Level` shape, tile model, win check | 02 |
| Camera scroll/clamp bound `[0, w*tileSize - viewport]` | 02 (draw + cull math in 09) |
...
| `sprites.json` + `sprite` anim state + `advanceSprite`; camera follow/cull | 09 |
| HUD element→source table + edge-arrow compass math | 09 |
